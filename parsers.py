    ...
    with open(file) as f:
        for index, line in (line_enum := enumerate(f)):
            ...
            if __class:
                ...
                if line.startswith("*/"):
                    idx, raw = next(line_enum)
                    raw = raw.strip().split(" ")
                    return_type = raw[0]
                    function = raw[1].split("(")[0]
...
def generate_lua_code(parsed_source: dict[str, dict[str, dict[str, None | str | list]]]):
    ...
    for enum, items in parsed_source['enums'].items():
        ...
        for value, e_param in enumerate(items['params']):
            ...
            lua_source += f"    {e_param['param']} = {value}" + ("," if value != len(items['params']) - 1 else "") + "\n"
        lua_source += "}\n\n"
    ...
    for _class, methods in parsed_source['classes'].items():
        ...
        for function, metadata in methods.items():
            ...
            for param in metadata['params']:
                if param:
                    lua_source += f"---@param {param['param']} any {param['desc']}\n"
